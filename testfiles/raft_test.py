...
predicted_flows = torch.cat(
    [predicted_flows.cpu(), torch.from_numpy(flow_1).unsqueeze(0), torch.from_numpy(flow_2).unsqueeze(0)], dim=0)
...
grid = [[img1, flow_img, flow_img_cv] for (img1, flow_img, flow_img_cv) in
        zip(img1_batch, predicted_flows_dl, predicted_flows_cv)]
# plot(grid)
